        ...
        with lmdb.open(lmdb_path, map_size=1024 ** 4, readahead=False) as env:
            with env.begin(write=True) as txn:
                ...
                with multiprocessing.Pool(n_worker) as pool:
                    for idx, result, filename in tqdm(pool.imap_unordered(resizer, enumerate(filenames), chunksize=chunksize),total=total):
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--3dmm_vid_path', type=str, help='a path to output directory', default=None)
