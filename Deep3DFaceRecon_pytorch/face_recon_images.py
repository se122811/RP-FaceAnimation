...
def main(opt, model):
    ...
    for iteration, data in tqdm(enumerate(dataloader)):
        ...
        img_result_path = "/data_2/Deep3DFaceRecon_pytorch/inference_CelebA/img_result/test"
        ...
        for name in visuals:
            ...
            for i in range(images.shape[0]):
                ...
                img_name = os.path.basename(data['filename'][i])+'.png'
                save_image(image_numpy, os.path.join(img_result_path, img_name))
                            

    # pred_coeffs = np.concatenate(pred_coeffs, 0)
    # pred_trans_params = np.concatenate(pred_trans_params, 0)
    # savemat( '/data_2/Deep3DFaceRecon_pytorch/inference_CelebA/CelebA_test.mat', {'coeff':pred_coeffs, 'transform_params':pred_trans_params})
