...
class BaseTrainer(object):
    ...
    def __init__(self,
       ...
       ##distributed 하는 방법 다시 생각하기  
        if opt.distributed:
            self.net_G_module = self.net_G.module
            self.net_D_module = self.net_D.module
        else:
            self.net_G_module = self.net_G
            self.net_D_module = self.net_D
        ...
        self.freeze_network(self.net_G)
        self.nofreeze_network(self.net_D)
    
            
    ## mapping, warpping freeze 시키기. 
    def freeze_network(self, model):
       for name, p in model.named_parameters():
           if 'warpping_net' in name :
               p.requires_grad = False
           elif 'mapping_net' in name:
               p.requires_grad = False
    
    def nofreeze_network(self, model):
           for name, p in model.named_parameters():
    ...
    def _load_checkpoint(self, checkpoint_path, checkpoint_path_D, resume=True):
        #Generator
        checkpoint_G = torch.load(checkpoint_path, map_location=lambda storage, loc: storage)
        self.net_G.load_state_dict(checkpoint_G['net_G'], strict=False)
        self.net_G_ema.load_state_dict(checkpoint_G['net_G_ema'], strict=False)
        print('load [net_G] and [net_G_ema] from {}'.format(checkpoint_path))
        
    
        #Discriminator
        # checkpoint_D = torch.load(checkpoint_path_D, map_location=lambda storage, loc: storage)
        # self.net_D.load_state_dict(checkpoint_D['net_D'], strict=False)
        # self.net_D_ema.load_state_dict(checkpoint_D['net_D_ema'], strict=False)
        # print('load [net_D] and [net_D_ema] from {}'.format(checkpoint_path_D))

        if resume or self.opt.phase == 'test':
            current_epoch = checkpoint_G['current_epoch']
            current_iteration = checkpoint_G['current_iteration']
...
def _save_checkpoint(opt,
    ...
    latest_checkpoint_path_D = 'Discriminator_epoch_{:05}_iteration_{:09}_checkpoint.pt'.format(
        current_epoch, current_iteration)
    save_path_D = os.path.join(opt.logdir,latest_checkpoint_path_D)
    ...
    torch.save(
        {
            ...
            'net_D_ema': net_D_ema.state_dict(),
