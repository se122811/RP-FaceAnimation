...
class BaseTrainer(object):
    ...
    def _load_checkpoint(self, checkpoint_path, resume=True):
        ...
        #### editing 빼고 가지고 오기 #####
        # pre_checkpoint = torch.load(checkpoint_path, map_location=lambda storage, loc: storage)
        # pre_checkpoint["net_G"] = {k: v for k,v in pre_checkpoint["net_G"].items() if "editing_net" not in k}
        # pre_checkpoint["net_G_ema"] = {k: v for k,v in pre_checkpoint["net_G_ema"].items() if "editing_net" not in k}
        # self.net_G.load_state_dict(pre_checkpoint['net_G'], strict=False)
        # self.net_G_ema.load_state_dict(pre_checkpoint['net_G_ema'], strict=False)
