...
from util.trainer_gan import get_model_optimizer_and_scheduler, set_random_seed, get_trainer
...
def parse_args():
    ...
    parser.add_argument('--config', default='./config/face_gan.yaml')
...
if __name__ == '__main__':
    ...
    # create a model generator and discriminator
    net_G, net_G_ema, opt_G, sch_G, net_D, net_D_ema, opt_D, sch_D \
        = get_model_optimizer_and_scheduler(opt)

    trainer = get_trainer(opt, net_G, net_G_ema, opt_G, sch_G, net_D, net_D_ema, opt_D, sch_D, train_dataset)
    
    # latest_checkpoin.txt 파일에 마지막으로 적혀있는 에폭 파일 불러와서 학습시킴. 
