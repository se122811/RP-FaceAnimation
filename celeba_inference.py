...
from data.celeba_video_dataset import CelebAVideoDataset
...
if __name__ == '__main__':
    ...
    img_output_dir = args.image_output_dir

    


    os.makedirs(output_dir, exist_ok=True)
    dataset = CelebAVideoDataset(opt.data, is_inference=True)

    with torch.no_grad():
        for video_index in range( dataset.__len__()):
            data = dataset.load_next_video()
            input_source = data['source_image'][None].cuda()
            input_3dmm_source = data['img_3dmm_source'][None].cuda()
            ...
            for frame_index in range(len(data['target_semantics'])):
                ...
                output_dict = net_G(input_source, input_3dmm_source, target_semantic)
            ...
            write2image(img_output_dir, name, gen_images[0])
            # write2video("{}/{}".format(output_dir, name), gt_images, warp_images, gen_images)
            print("write results to video {}/{}".format(img_output_dir, name))
