...
from generators.base_function import LayerNorm2d, ADAINHourglass, FineEncoder, FineDecoder, FineUVEncoder, FineUVDecoder, Transformer_encoder, Transformer_decoder
...
class FaceGenerator(nn.Module):
    ...
    def uvmap_to_render(self, uv_map, input_image, pred_vertex, face_index, pred_mask, uv_h =256, uv_w=256, uv_c = 3, show_flag=False, wfp=None):     
        ...
        self.rerender_face =  self.rerender_face * pred_mask + (1 - pred_mask) * 0
    ...
    def forward(
        ...
        if stage == 'warp':
            ...
            rerender_output_vis_syn, self.face_render_syn = self.uvmap_to_render(input_uv_map, output['warp_image'], self.pred_vertex, self.face_index, pred_mask)  #syn rendering image
            ...
            ### editing network ###
            ###### concat input and rendering image ######
            ############ name : face_re ############
            ...
            ###### concat warp and rendering image ######
            ...
            output['fake_image'] = self.editing_net(input_image, self.face_render)
...
class EditingNet(nn.Module):
    ...
    def forward(self, warp_image, render):

        x = self.transformer_encoder(warp_image, render)
        result= self.transformer_decoder(x)
...
#     def forward(self, input_image, warp_image, descriptor):
#         if input_image.shape[0] < warp_image.shape[0]:
#             #concat warp
#             #inference
#             if input_image.shape[0] == 1:
#                 warp_image_repeat = warp_image[0].repeat(10,1,1,1)
#                 warp_image_rendering = warp_image[1].repeat(10,1,1,1)
#                 warp_image_f = torch.cat([warp_image_repeat, warp_image_rendering],0)
                
#                 warp_image_trans = warp_image_f.permute(1,2,3,0)
#                 warp_image_fc = self.temp(warp_image_trans)
#                 warp_image_reverse = warp_image_fc.permute(3,0,1,2)
                
#                 x = torch.cat([input_image, warp_image_reverse[0].unsqueeze(0)], 1)
#                 x = self.encoder(x)
#                 gen_image = self.decoder(x, descriptor)
#                 return gen_image
...
#             #train
#             else:
#                 warp_image_trans = warp_image.permute(1,2,3,0)
#                 warp_image_fc = self.temp(warp_image_trans)
#                 warp_image_reverse = warp_image_fc.permute(3,0,1,2)
#                 x = torch.cat([input_image, warp_image_reverse], 1)
#                 x = self.encoder(x)
#                 gen_image = self.decoder(x, descriptor)
#                 return gen_image
...
#                 x = torch.cat([input_image_reverse[0].unsqueeze(0), warp_image], 1)
#                 x = self.encoder(x)
#                 gen_image = self.decoder(x, descriptor)
#                 return gen_image
