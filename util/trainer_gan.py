...
def accumulate(model1, model2, model1_D, model2_D, decay=0.999):
    par1 = dict(model1.named_parameters())
    par2 = dict(model2.named_parameters())
    par1_D = dict(model1_D.named_parameters())
    par1_D = dict(model2_D.named_parameters())

    for k in par1.keys():
        par1[k].data.mul_(decay).add_(par2[k].data, alpha=1 - decay)
    
    for k in par1_D.keys():
        par1_D[k].data.mul_(decay).add_(par1_D[k].data, alpha=1 - decay)
...
def get_trainer(opt, net_G, net_G_ema, opt_G, sch_G, net_D, net_D_ema, opt_D, sch_D, train_dataset):
    ...
    trainer = trainer_class(opt, net_G, net_G_ema, opt_G, sch_G, net_D, net_D_ema, opt_D, sch_D, train_dataset)
...
def get_model_optimizer_and_scheduler(opt):
    ...
    ## gen_module, gen_network_name 뭔지 확인해보기. 
    ## generator
    ...
    lib_D = importlib.import_module(dis_module)
    network_D = getattr(lib_D, dis_network_name)
    
    # face generator 로 들어가는 부분
    net_G = network(**opt.gen.param).to(opt.device)
    net_D = network_D(**opt.dis.param).to(opt.device)
    ...
    net_D_ema = network(**opt.gen.param).to(opt.device)
    net_D_ema.eval()
    
    
    accumulate(net_G_ema, net_G, net_D_ema, net_D, 0)
    ...
    return net_G, net_G_ema, opt_G, sch_G, net_D, net_D_ema, opt_D, sch_D
